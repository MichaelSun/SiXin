	...
	public void put(int hashcode, Emotion emotion) {
		...
		try {
			...
			if (mEmotionTable != null) {
				Node node = new Node(hashcode, emotion.counter);
				...
				if (ret == -1) {// 不够资格进入缓存
					...
					if (mEmotionTable.containsKey(ret)) {// /这一步可以删掉
						...
						CommonUtil.log("xiaochao", "ret " + ret + "size "
								+ mEmotionTable.size());
						CommonUtil.log("xiaochao", "Cache 同步机制出现问题 " + hashcode
								+ "name " + emotion.mEmotionName);
					}
				}
			}
...
//					+ "  list size: " + list.size());
	 ...
	 * @return
	 */
	private int addToList(Node node){///添加后进行排序
	...
	private Emotion getFromLocal(int key){
		...
		if(plist != null){
			for(int i = 0; i < plist.size(); ++i){
				com.common.emotion.emotion.Node nodeTmp = EmotionNameList.getEmotionNameList(plist.get(i));
				if(nodeTmp== null){
					return null;
				}
...
//										EmotionNameList.updateCounter(plist.get(i), index, emotion.counter+=1);
	...
	private class Node{
		public Node(int code, int counter) {
			this.code = code;
