...
import com.common.R;
import com.sixin.widgets.interpolator.DecelerateInterpolator;
import com.sixin.widgets.layout.scalable.IScalable;
...
public final class FakeChromeLayout extends ViewGroup implements IFakeChrome {
    ...
    public static final int PAGE_INDICATOR_ID = R.id.fake_chrome_indicator;
    ...
    private static final boolean CAN_ALPHA = Build.VERSION.SDK_INT > Build.VERSION_CODES.HONEYCOMB;
...
//            Log.d(TAG, "@compute");
    ...
    private class PagerTracker {
        ...
        void move(int x) {
            ...
            changed = (lastDistance < 0 && distance >= 0) || (lastDistance > 0 && distance <= 0);
            ...
            mIndicator.setSelectorPosition(mCurItem - mCurView.getLeft() / (float) getWidth());
            invalidate();
        }
    ...
    private class OverviewTracker {
        ...
        void prepareVerticalTracking() {
            ...
            maxHeight = (getHeight() - mHeightPadding) / (mInfos.size() == 1 ? 1 : mInfos.size() - 1);
        }
...
//            Log.d(TAG, "@computeFling " + offset);
            ...
            for (ItemInfo ii : mInfos) {
                child = getChildView(ii);
                top = child.getTop();
                if (offset < 0 && top + offset * index < mOverviewTracker.minHeight * index) {
                    child.offsetTopAndBottom(mOverviewTracker.minHeight * index - top);
                } else if (offset > 0 && top + offset * index > mOverviewTracker.maxHeight * index) {
...
//        Log.d(TAG, "@onLayout");
        ...
        if (mMode == MODE_PAGER) {
            ...
            if (mIndicator != null) {
                ...
                mIndicator.layout((width - indicatorWidth) / 2, height - indicatorHeight,
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (child != null && child.getLayoutParams() != null) {
                    child.layout(l, i * perHeight, r, i * perHeight + child.getMeasuredHeight());
                }
            }
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        return mAdapter == null || mAdapter.getCount() == 0 || mPagerAnimator.animating ||
                mOverviewAnimator.animating || mSwitchAnimator.animating ||
                mOverviewLayoutAnimator.animating || mAdditionDeletionAnimator.animating ||
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (!mPagerTracker.tracking &&
                            (x < mLastDownX - mTapThreshold || x > mLastDownX + mTapThreshold)) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mMode == MODE_PAGER) {
            ...
            switch (action) {
                case MotionEvent.ACTION_MOVE:
                    if (!mOverviewTracker.verticalTracking && !mOverviewTracker.horizontalTracking) {
                        if ((y < mLastDownY - mTapThreshold || y > mLastDownY + mTapThreshold)) {
                            mEventMoved = true;
                            mLastOffsetDownY = y;
                            mOverviewTracker.prepareVerticalTracking();
                        } else if ((x < mLastDownX - mTapThreshold || x > mLastDownX + mTapThreshold)) {
                            mLastOffsetDownX = x;
                            mEventMoved = true;
                ...
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    if (!mEventMoved && x < mLastDownX + mTapThreshold && x > mLastDownX - mTapThreshold &&
                            y < mLastDownY + mTapThreshold && y > mLastDownY - mTapThreshold) {
                        mSwitchAnimator.animatePager();
                        return true;
                    }
                    if (mOverviewTracker.verticalTracking) {
...
//                Log.d(TAG, "@dispatchDraw");
        ...
        if (mIndicator != null && (mPagerAnimator.animating || mPagerTracker.tracking)) {
