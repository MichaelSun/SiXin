	...
	public static boolean toFile(File file, byte[] data) {
		...
		try{
			while(size<data.length) {
				int length = (data.length - size>buffer.length) ? buffer.length : (data.length-size);//每次复制的byte数
...
//			GlobalValue.getRootActivity().startActivity(intent);
	...
	public static int getTextSizeOnPaint(int textSize){
		Context c = AbstractRenrenApplication.getAppContext();
		Resources r;
		if (c == null)
	...
	public static int getSizeOnDevice(int textSize){
		...
		if (c == null)
		    ...
		    r = c.getResources();
		return  (int)TypedValue.applyDimension(
		        TypedValue.COMPLEX_UNIT_DIP, textSize, r.getDisplayMetrics());
}
        ...
        try {
            ...
            while((line = br.readLine()) != null){
                if(line.indexOf("Serial") > -1){
                    strSerial = (line.substring(line.indexOf(":")+1, line.length())).trim();
                    break;
                }
                line = null;
            }
            ...
            CommonUtil.log("yxb", "NumberFormatException");
        }
    }
    ...
    public static String[] parserBracketsPoints(String content){
    	...
    	if(content.length()>1&&(content.length()-1)>0){
    		...
    		if(index!=-1){
    			...
    			str[1]	 = childContent.substring(index+1,childContent.length());
    			CommonUtil.log("Methods",str[0]+"|"+str[1]+"  subString:"+ childContent);
    			return str;
    		}
    		else{
    			return null;
