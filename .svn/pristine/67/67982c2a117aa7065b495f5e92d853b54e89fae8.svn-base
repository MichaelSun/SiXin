...
public class CommonUtil {
	...
	public static void toast(final String msg){
		AbstractApplication.HANDLER.post(new Runnable() {
			@Override
			public void run() {
	...
	public static void toast(final int msgId){
		AbstractApplication.HANDLER.post(new Runnable() {
			public void run() {
				...
				if(!TextUtils.isEmpty(AbstractApplication.getAppContext().getResources().getText(msgId))){
					Toast.makeText(
							AbstractApplication.getAppContext(), 
							AbstractApplication.getAppContext().getResources().getText(msgId), 
							Toast.LENGTH_SHORT).show();
				}
...
//		Log.v(logName, "&&&E:"+logMessage);
	...
	/** add by jia.xia
	 * 转换成DIP
	 * @param number  ：1.0密度下的值
	 * @return
	 */
	public static int calcFromDip(int number){
		float f = AbstractRenrenApplication.metric.density;
		return (int)(number*f);
	}
