 ...
 * @author dingwei.chen
 * @说明 对象级别的表结构
 * 		所有继承于该类的对象必须在类前注入 {@link com.renren.mobile.chat.base.annotation.Table}
 * @说明 在生成一个表对象的时候必须指定建立在这张表上的{@link com.renren.mobile.chat.base.database.DAO}
...
public abstract class BaseDBTable {
	...
	public String getCreateTableString(){
		...
		sbuilder.append(" CREATE TABLE "+getTableName() +"(");
		try {
			for(Field f:fields){
				f.setAccessible(true);
				Column c = f.getAnnotation(Column.class);
				if(c!=null){
					sbuilder.append(f.get(clazz).toString()+" "+c.defineType());
					sbuilder.append(",");
				}
			}
			if(fields.length>0){
