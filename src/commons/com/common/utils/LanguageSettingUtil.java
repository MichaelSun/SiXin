...
public class LanguageSettingUtil {
	...
	private LanguageSettingUtil() {
		mSP = new RRSharedPreferences(AbstractApplication.getAppContext(),SP_NAME);
	}
	...
	public String getLanguage() {
		...
		if ("com.renren.mobile.chat".equals(AbstractRenrenApplication
			...
			if (TextUtils.isEmpty(str)) {
				str = getLocaleString(Config.DEFUALT_LOCALE);
			}
		}
		return str;
	}
...
//	国际版调用
	public void updateConfiguration(){
		Configuration config = AbstractRenrenApplication.getAppContext().getResources().getConfiguration();
		config.locale=getLocale();
		AbstractRenrenApplication.getAppContext().getResources().updateConfiguration(config, null);
	}
}
