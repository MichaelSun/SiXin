...
public class BaseImageView extends ImageView {
	...
	public FrameLayout.LayoutParams mDefaultParams = 
			new FrameLayout.LayoutParams(
					GlobalValue.getInstance().calcFromDip(DEFAULT_PARAMS_WIDTH_MID)
					,GlobalValue.getInstance().calcFromDip(DEFAULT_PARAMS_HEIGHT_MID));
...
//		canvas.drawBitmap(mBitmap, null, getRect(), null);
	...
	public void setImagePath(String path,boolean isDefault_Left){
		...
		if(file.exists()){
			...
			if(bitmap!=null){
				...
				/*图片丢失*/
				this.setImageBitmap(GlobalValue.getInstance().getDefualtBitmap(isDefault_Left));
				return;
			}
		}else{
			/*图片丢失*/
	...
	public Size calcParams(Bitmap bitmap){
		...
		int width = GlobalValue.getInstance().calcFromDip(bitmap.getWidth());
		int height = GlobalValue.getInstance().calcFromDip(bitmap.getHeight());
		int viewWidth = size.width;
		int viewHeight = size.height;
		int warnNumber = GlobalValue.getInstance().calcFromDip(WARN_LENGTH_MID);
		if(width< warnNumber && height<warnNumber){
			size.width = width;
			size.height = height;
			return size; 
		}
		int min_length =  GlobalValue.getInstance().calcFromDip(MIN_LENGTH_MID);
		if(width>height){
