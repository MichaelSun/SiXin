...
public class RenrenPhotoImageView extends ImageView {
    ...
    public RenrenPhotoImageView(Context context, int imageWidth, int imageHeight) {
        super(context);
    ...
    public RenrenPhotoImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void arithScaleRate() {
        float scaleWidth ;
        float scaleHeight;
        scaleWidth = PhotoNew.screenWidth / (float) imageWidth;
        scaleHeight = PhotoNew.screenHeight / (float) imageHeight;
        scaleRate = Math.min(scaleWidth, scaleHeight);
        mMinZoom = scaleRate;
        mMaxZoom = scaleRate * 4.0f;
//        Log.v("@@@","arithScaleRate screenWidth:" + PhotoNew.screenWidth
//              + " and screenHeight:" + PhotoNew.screenHeight
//                + " and scaleRate:" + scaleRate
//                + " and imageWidth:" + imageWidth
//                + " and imageHeight:" + imageHeight
//                + " and scaleWidth:" + scaleWidth
//                + " and scaleHeight:" + scaleHeight);
    }
...
//        Log.v("@@@","RenrenPhotoImageImageView setImageBitmap");
        super.setImageBitmap(bitmap);
        image = bitmap;
        // 计算适应屏幕的比例
        arithScaleRate();
        //缩放到屏幕大小
        zoomTo(scaleRate,PhotoNew.screenWidth / 2f, PhotoNew.screenHeight / 2f);
...
//        Log.v("@@@","center image.getWidth() is :" + image.getWidth() + "image.getHeight is:" +image.getHeight() +
//             "");
        m.mapRect(rect);
...
//            int viewHeight = getHeight();
            if (height <= PhotoNew.screenHeight) {
                deltaY = (PhotoNew.screenHeight - height) / 2 - rect.top;
            } else if (rect.top > 0) {
                deltaY = -rect.top;
            } else if (rect.bottom < PhotoNew.screenHeight) {
                deltaY = PhotoNew.screenHeight - rect.bottom;
            }
        }

        if (horizontal) {
//            int viewWidth = getWidth();
            if (width <= PhotoNew.screenWidth) {
                deltaX = (PhotoNew.screenWidth - width) / 2 - rect.left;
            } else if (rect.left > 0) {
                deltaX = -rect.left;
            } else if (rect.right < PhotoNew.screenWidth) {
                deltaX = PhotoNew.screenWidth - rect.right;
            }
        }
...
//    }
    ...
    protected void zoomTo(float scale, float centerX, float centerY) {
        if (scale > mMaxZoom) {
//            Log.v("@@@","scale > mMaxZoom");
            scale = mMaxZoom;
        } else if (scale < mMinZoom) {
//            Log.v("@@@","scale < mMinZoom");
            scale = mMinZoom;
        }

        float oldScale = getScale();
//        Log.v("@@@","zoomTo oldScale is:" + oldScale);
//        Log.v("@@@","zoomTo scale is:" + scale);
        float deltaScale = scale / oldScale;
//        Log.v("@@@","zoomTo deltaScale is:" + deltaScale);
