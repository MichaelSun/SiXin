...
import view.test.TestImageView;
...
//    <attr format="boolean" name="attach_right"/>
	...
	void initViews(){
		...
		mErrorView = new TestImageView(getContext());
		...
		for(View v:views){
			if(v!=null){
				if(v.getLayoutParams()==null){
					SystemUtil.log("layout1", "add view1 ="+v.getClass().getSimpleName());
					...
					SystemUtil.log("layout1", "add view2 ="+v.getClass().getSimpleName());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(mDomainView!=null){
			...
			SystemUtil.log("domain", "domain is not null "+lw);
			lw+=mLeft_Padding;
		}
	...
	void layoutRight(int left, int top, int right,int bottom){
		...
		SystemUtil.log("log", "layoutRight = "+(mErrorView.getVisibility()==View.VISIBLE));
		int height = this.getHeight();
