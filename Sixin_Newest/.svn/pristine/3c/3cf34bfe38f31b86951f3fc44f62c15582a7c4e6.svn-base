		...
		if(deltaTime<=ONE_DAY*3){
			if(deltaTime>ONE_DAY<<2){
				...
				if(deltaTime<=ONE_DAY){
					Log.v("vava", "deltaTime = " + deltaTime);
					Log.v("vava", "ONE_DAY = " + ONE_DAY);
					Log.v("vava", "1111");
					if(deltaTime < 0){
						return RenrenChatApplication.getmContext().getResources().getString(R.string.ChatSessionAdapter_java_3) + "Special";		
					}
					return RenrenChatApplication.getmContext().getResources().getString(R.string.ChatSessionAdapter_java_3);//ChatSessionAdapter_java_3=昨天; 
				}else{
